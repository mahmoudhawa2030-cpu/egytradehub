...
export default function SignupPage() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    // 2. Create profile row (trigger will handle this in production, but we do it explicitly for now)
    ...
    if (profileError) {
      // Don't fail signup if profile insert fails — user can complete profile later
      ...
      <div className="min-h-screen bg-neutral-100 flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-white rounded-3xl shadow-lg p-8 text-center">
          ...
          <p className="text-neutral-600">
            Welcome to TradeHub. Redirecting you to your dashboard...
          </p>
        </div>
      </div>
    );
      ...
      <div className="w-full max-w-md bg-white rounded-3xl shadow-lg p-8">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-neutral-500 mt-2">
            Join TradeHub to buy or sell wholesale
          </p>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { id: "buyer", label: "Buy products", icon: User },
              { id: "supplier", label: "Sell products", icon: Building2 },
              { id: "admin", label: "Manage", icon: Building2 },
            ].map(({ id, label, icon: Icon }) => {
              const active = form.role === (id as ProfileRole);
              return (
                <button
                  key={id}
                  type="button"
                  onClick={() => update("role", id as ProfileRole)}
                  className={[
        ...
        <Link
          href="/"
          className="block text-center text-sm text-neutral-400 mt-4 hover:text-neutral-600"
        >
          ← Back to home
        </Link>
      </div>
