...
export default function SupplierEditProductForm({ product, categories }: { product: Product; categories: string[] }) {
        ...
        <div className="bg-white rounded-xl border border-neutral-200 p-6 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" className="sr-only peer" checked={isFlash} onChange={(e) => setIsFlash(e.target.checked)} />
          ...
          {isFlash && (
            <div className="grid grid-cols-3 gap-4 pt-2">
              <div>
                ...
                <input name="flash_discount_pct" type="number" min="1" max="99" defaultValue={product.flash_discount_pct ?? ""}
                  className="w-full px-4 py-2 border border-neutral-200 rounded-lg focus:outline-none focus:border-[#FF6A00]" />
              </div>
              <div>
                <label className="block text-sm font-medium text-neutral-700 mb-1.5">Starts At</label>
                <input name="flash_starts_at" type="datetime-local" defaultValue={product.flash_starts_at?.slice(0, 16) ?? ""}
                  className="w-full px-4 py-2 border border-neutral-200 rounded-lg focus:outline-none focus:border-[#FF6A00]" />
              </div>
              <div>
                <label className="block text-sm font-medium text-neutral-700 mb-1.5">Ends At</label>
                <input name="flash_ends_at" type="datetime-local" defaultValue={product.flash_ends_at?.slice(0, 16) ?? ""}
                  className="w-full px-4 py-2 border border-neutral-200 rounded-lg focus:outline-none focus:border-[#FF6A00]" />
              </div>
            </div>
          )}
